...
#+end_src
** 解法
バイナリから ~xor_flag~ の値を抽出して、各バイトを再度 7 と排他的論理和をとれば元のフラグが得られる。

*** 推測
まずは ~Alpaca{}~ が含まれていると考えて、変換してみる。

#+begin_src ruby :tangle demo.rb :results value pp :exports both
  def main
    p "Hello!"
...
#+begin_src ruby :tangle solver.rb :results value pp :exports both
  def main
